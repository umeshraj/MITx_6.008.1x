...
def compute_posterior(prior, likelihood, y):
    ...
    log_like= np.log(likelihood[y, :])
    num = log_prior + np.sum(log_like, axis=0)
...
def infer_true_movie_ratings_fast(movie_id_list, ratings):
    """
    For every movie, computes the posterior distribution and MAP estimate of
    the movie's true/inherent rating given the movie's observed ratings.
    ...
    # These are the output variables - it's your job to fill them.
    num_movies = len(movie_id_list)
    posteriors = np.zeros((1, M))
    MAP_ratings = np.zeros(1)

    movie_posterior = compute_posterior(prior, likelihood, ratings)
    posteriors[0, :] = movie_posterior
    MAP_ratings[0] = np.argmax(movie_posterior)
...
def compute_entropy(distribution):
    ...
    # - use log base 2
    # - enforce 0log0 = 0
    distribution = distribution[distribution > 1e-8]
    entropy = distribution * np.log2(distribution)
...
def compute_true_movie_rating_posterior_entropies_fast():
    ...
    for movie_id in movie_id_list:
        ...
        for nObs in range(1, N+1):
            ...
            posterior, _ = infer_true_movie_ratings_fast(movie_id_list, 
                                                   ratings)
